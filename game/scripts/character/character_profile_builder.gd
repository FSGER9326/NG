...
static func build_profile(character_name: String, ancestry: Dictionary, background: Dictionary, character_class: Dictionary, trait: Dictionary, creation_data: Dictionary) -> Dictionary:
	...
	var portrait_id := _portrait_id_for_background(background)
	var profile := {
		"name": clean_name,
		...
		"portrait": _portrait_name_for_id(portrait_id, creation_data),
		"tags": [],
	...
	if portrait_id.is_empty():
		profile["compatibility_warnings"].append("%s is missing a portrait_id" % String(background.get("name", "Background")))
...
static func _portrait_id_for_background(background: Dictionary) -> String:
	return String(background.get("portrait_id", ""))

static func _portrait_name_for_id(portrait_id: String, creation_data: Dictionary) -> String:
	if portrait_id.is_empty():
		return ""
	var portraits: Variant = creation_data.get("portraits", [])
	if typeof(portraits) == TYPE_ARRAY:
		for portrait in portraits:
			if typeof(portrait) == TYPE_DICTIONARY and String(portrait.get("id", "")) == portrait_id:
				return String(portrait.get("name", portrait_id))
	return _fallback_portrait_name_from_id(portrait_id)

static func _fallback_portrait_name_from_id(portrait_id: String) -> String:
	var cleaned := portrait_id.trim_prefix("portrait_")

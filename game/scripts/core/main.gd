...
func _show_character_creator() -> void:
	_clear_current_screen()
	character_creator_root = Control.new()
	character_creator_root.name = "CharacterCreator"
	character_creator_root.set_anchors_preset(Control.PRESET_FULL_RECT)
	...
	name_edit.text = String(player_profile.get("name", "Wanderer"))
	...
	var origin_options := OptionButton.new()
	...
	var archetype_options := OptionButton.new()
	...
	start_button.pressed.connect(_start_new_game_from_creator.bind(name_edit, origin_options, archetype_options))
	buttons.add_child(start_button)
	GameLog.info("MENU", "Character creator shown")

func _start_new_game_from_creator(name_edit: LineEdit, origin_options: OptionButton, archetype_options: OptionButton) -> void:
	var character_name := name_edit.text.strip_edges()
	if character_name.is_empty():
		character_name = "Wanderer"
	player_profile = {
		"name": character_name,
		"origin": origin_options.get_item_text(origin_options.selected),
		"archetype": archetype_options.get_item_text(archetype_options.selected)
	}

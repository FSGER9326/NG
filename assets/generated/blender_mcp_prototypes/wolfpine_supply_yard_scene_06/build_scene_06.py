...
OUT_DIR = ROOT / "assets" / "generated" / "blender_mcp_prototypes" / "wolfpine_supply_yard_scene_06"
...
try:
    scene.eevee.taa_render_samples=96; scene.eevee.use_gtao=True; scene.eevee.gtao_distance=4.0; scene.eevee.gtao_factor=1.85
except Exception: pass
scene.render.resolution_x=1600; scene.render.resolution_y=900; scene.render.film_transparent=False
scene.render.image_settings.file_format='PNG'; scene.render.image_settings.color_mode='RGBA'
scene.view_settings.view_transform='Standard'; scene.view_settings.look='Medium High Contrast'; scene.view_settings.exposure=-0.16; scene.view_settings.gamma=1.0
scene.world.color=(0.045,0.052,0.046)
...
mud=img_mat('mud_texture_real','mud_flecked.png',.93,.82)
dirt=img_mat('packed_dirt_real','packed_dirt_streaked.png',.93,.88)
grass=img_mat('moss_grass_real','dark_grass_moss.png',.95,.95)
stone=img_mat('worn_stone_real','worn_stone.png',.91,.82)
wood=img_mat('weathered_wood_real','weathered_plank_wood.png',.90,.78)
wood_dark=img_mat('dark_weathered_wood_real','weathered_plank_wood.png',.92,.44)
roof=img_mat('mossy_roof_real','mossy_slate_roof.png',.94,.75)
plaster=img_mat('dirty_plaster_real','dirty_plaster.png',.93,.78)
sack=img_mat('sackcloth_real','sackcloth_weave.png',.96,.80)
parch=img_mat('parchment_real','damp_parchment.png',.92,.84)
iron=flat('dull_black_iron',(0.045,0.044,0.041,1),.95)
shadow=flat('transparent_soft_shadow',(0.017,0.014,0.012,.28),1,.28)
wax=flat('muted_wax',(0.28,.05,.042,1),.9)
dark=flat('near_black_recess',(0.020,0.017,0.014,1),.95)
...
# Lighting/camera.
bpy.ops.object.light_add(type='AREA',location=(-4.8,-5.2,7.5)); key=bpy.context.object; key.name='soft_upper_left_daylight'; key.data.energy=390; key.data.size=8.5
bpy.ops.object.light_add(type='POINT',location=(-1.1,1.2,1.6)); fill=bpy.context.object; fill.name='warm_storehouse_bounce'; fill.data.energy=10; fill.data.color=(1,.60,.33)
...
scene.render.filepath=str(OUT_DIR/'wolfpine_supply_yard_scene_06_raw.png'); bpy.ops.render.render(write_still=True); bpy.ops.wm.save_as_mainfile(filepath=str(OUT_DIR/'wolfpine_supply_yard_scene_06.blend'))
print(f"WROTE {OUT_DIR / 'wolfpine_supply_yard_scene_06_raw.png'}")
print(f"WROTE {OUT_DIR / 'wolfpine_supply_yard_scene_06.blend'}")
